  ...
  return (
    <thead>
      <tr class="text-left text-xs font-semibold uppercase tracking-wide text-slate-400 dark:text-slate-500 border-b border-slate-100 dark:border-slate-800">
        <th class="px-3.5 py-2 font-semibold w-8"></th>
        <For each={["Name", "Iface", "IP", "MAC", "Hardware", "Date", "Known", "On"]}>{(key) =>
          <th
            class={"px-3.5 py-2 font-semibold select-none " + (key === sortField() ? "text-brand-600 dark:text-brand-400" : "")}
          >
            <button
              type="button"
        ...
        <th class="px-3.5 py-2 font-semibold w-8 text-right" title="Bearbeiten">
